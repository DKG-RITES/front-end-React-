      ...
      complete: async (results) => {
        ...
        for (const row of rows) {
          try {
            ...
            console.log('Successfully saved row:', row);
          } catch (error) {
            console.error('Failed to save row:', row, error);
            message.error(`Failed to upload row with serialNumber: ${row.serialNumber}`);
          }
        }
  ...
  return (
    <FormContainer>
      ...
      <GeneralInfo data={calibrationGeneralInfo} />
          ...
          <Button loading={loading} icon={<UploadOutlined />}>Add Calibration From CSV</Button>
        </Upload>
