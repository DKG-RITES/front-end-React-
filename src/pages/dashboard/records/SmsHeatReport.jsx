...
const SmsHeatReport = () => {
  ...
  function filterByShift(data, targetShift) {
  return data.filter(item => {
    const shift = item.dateAndShiftOfCasting.split(' ').pop();  // Extract shift
  ...
  const populateData = async () => {
    ...
    try {
      ...
      if(filter?.shift){
        const resp = filterByShift(data?.responseData, filter.shift)
        setDataSource(resp)
      }
      else{
      ...
      <div className="mt-4 border border-darkBlueHover p-2 py-4">
        <div>
          <Form
            ...
            className="grid md:grid-cols-3 grid-cols-2 gap-x-2 items-center p-2 pt-0"
          >
            <CustomDatePicker
              className="no-border"
