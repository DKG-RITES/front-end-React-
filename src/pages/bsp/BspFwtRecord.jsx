...
const BspFwtRecord = () => {
  ...
  const populateData = async () => {
    try {
      let url = `/bsp/fwt/data?page=0&size=1000`;
      ...
      console.log("FWT API Response:", data);
      ...
      if (filter.startDate && filter.endDate) {
        ...
        console.log("All FWT data without date filtering");
      }
    } catch (error) {
      console.error("Error fetching FWT data:", error);
      message.error("Failed to fetch FWT data");
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-2 bg-white p-4 w-full md:w-4/5 mx-auto h-[100vh] md:h-fit">
      <SubHeader title="FWT Test Records" link="/bsp/data" />
